...
def build_learning_set(ds, seq_length=4, chunk_size=200, use_gpu=False):
    ...
    if "depth" in sst.dims:      #we select only sea surface depth
        sst = sst.isel(depth=0)     #check if 0 here is the index (+1) or the actual depth.

    sst = sst.astype("float32")      #we convert everything to float32 for model compatibility

    print(sst, flush=True)            #prints a summary of the dataset

    global_mean = float(sst.mean(skipna=True).compute())    #compute the mean of the variable and skips NA values for accuracy
    print(f"Global mean SST: {global_mean:.4f}", flush=True)

    X_chunks = []
    Y_chunks = []

    total_time = sst.sizes["time"]    #CHECK sst.sizes is a dictionary containing sizes of each dimension

    #\\TODO print size of each dimension of the ds


    for start in tqdm(     #CHECK what is tqdm and how the chunking works. why not use a torch dataloader?
        range(0, total_time - seq_length, chunk_size),
        desc="Processing chunks",
    ):
        end = min(total_time, start + chunk_size + seq_length)

        block_np = sst.isel(time=slice(start, end)).compute().values   #getting the block chunk in a numpy array
        block = torch.from_numpy(block_np).float()       #converting to a torch tensor of shape  (T, H, W)

        for t in range(block.shape[0] - seq_length):     #create the sequences for each element t in block
            ...
            X_t = neigh.contiguous().view(seq_length, -1, 9).permute(1, 0, 2) #CHECK how .view and .contiguous work in toch
