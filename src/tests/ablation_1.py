...
parser.add_argument(
    "--run-id",
    default=None,
    help="Evaluate this CPU run or SLURM job's checkpoints; omit for latest",
)
...
def latest_checkpoint(model_key, dataset, run_id=None):
    ...
    if run_id:
        f = MODEL_DIR / f"{CHECKPOINT_PREFIXES[model_key]}_1_{dataset}_{run_id}.pt"
        print(f)
...
for dataset in DATASETS:
    ...
    for model_key, model_class in models_to_eval.items():
        ...
        model_file = latest_checkpoint(model_key, dataset, args.run_id)
